...
class CompanyListTableViewController: UITableViewController {
    var selectedData = companyList()
    var companyData = [companyList]()
        ...
        let task = URLSession.shared.dataTask(with: url){(data,response,error) in
            ...
            if let data=data{
                do{
                    ...
                    let decodedData = try jsonData.decode(Array<companyList>.self, from: data)
                    self.companyData = decodedData
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        lblCompanyName.text = companyData[indexPath.row].company

        return cell
    }
    
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        self.selectedData = companyData[indexPath.row]
        performSegue(withIdentifier: "segueTableToMap", sender: self)
    }
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "segueTableToMap" {
            let seg = segue.destination as! ViewController
            seg.selectData = selectedData
        }
